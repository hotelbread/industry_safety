...
from src.gui_module.main_window import MainWindow  # 새 MainWindow 임포트
...
class MW(MainWindow):
    ...
    def __init__(self, app, parent=None):
        ...
        self.roi_threshold_sec = 5 # 체류 기준
    ...
    def reset_experience(self):
        ...
        # 화면 초기화
        self.instruction_panel.set_instruction("0") # 문구 변경
        self.progressbar_panel.reset_ui()
        self.eval_table.setup_table()
    ...
    def keyReleaseEvent(self, event):
        if event.key() == Qt.Key_Q:
            ...
            if self.admin_mode == True:
                ...
                self.canvas.admin_mode = False
                print("Admin mode deactivated")
                self.ai_thread.roi_points = self.canvas.roi_points
                print(f"[Debug][Main] Sent ROI points to AI thread: {self.ai_thread.roi_points}")
        else:
            pass
    ...
    def setup_thread(self):
        ...
        self.progressbar_panel.timer_finished.connect(self.next_stage)
        
        self.RankBoard.btn_reset.clicked.connect(self._on_reset_btn)
    ...
    #         self.id_last_seen[human_id] = now
    #         if human_id not in self.roi_enter_time:
    #             self.roi_enter_time[human_id] = now
    ...
    def handle_roi(self, detections):
        ...
        if len(ids_in_roi) > 0:
            ...
            if target_id is not None:
                ...
                if stay >= self.roi_threshold_sec:
                    self.active_user_id = target_id
                    if self.action_recognition == False:
                        self.progressbar_panel.start_timer(3, 'white', "start") # 5초 동작 인식 시작
                        self.canvas.set_visualization_mode(show_bbox=True, show_pose=True, show_action=False)
                        self.action_recognition = True
                    # print(f'[Debug][handle_roi] 새로운사람 등장 id : {self.active_user_id}')
        ...
        if self.active_user_id is not None: # active_user_id이 존재한다면
            ...
            # ids_in_roi_ids : id 만 추출해서 리스트
            ids_in_roi_ids = [x['id'] for x in ids_in_roi]
            # print(f'[Debug][handle_roi] ids_in_roi_ids {
    ...
    def update_scoretable(self, stage_result, row:int):
        ...
        acc, stab, avg = stage_result.conf_mean, stage_result.angle_score, stage_result.total_score
        self.eval_table.update_data(row, acc, stab, avg)
        """
        # index + 동작명
        ...
        # 안정성 = angle_score, 관절각도 점수
        result_ratio = QTableWidgetItem(f'{stage_result.angle_score:.1f}')
        ...
        # 평균 = total_score, = 정확도 *0.8 + 안정성 *0.2 (spine)
    ...
    def update_rank_table(self):
        """평가 테이블의 최종 점수를 계산하고 랭킹 보드에 등록"""
        try:
            # 1. 각 단계 점수 읽기
            scores = []
            for row in range(3): # 1, 2, 3단계 점수
                acc = float(self.eval_table.table.item(row, 1).text().replace('%', ''))
                stab = float(self.eval_table.table.item(row, 2).text().replace('%', ''))
                avg = float(self.eval_table.table.item(row, 3).text())
                scores.append({'acc': acc, 'stab': stab, 'avg': avg})

            # 2. 최종 평균 계산 (Accuracy, Stability, Total)
            final_acc = round(sum(s['acc'] for s in scores) / 3, 1)
            final_stab = round(sum(s['stab'] for s in scores) / 3, 1)
            final_total = round(sum(s['avg'] for s in scores) / 3, 1)

            # 3. 평가 테이블의 '최종' 행 업데이트
            # row=3은 '최종' 행 위치입니다.
            self.eval_table.update_data(3, final_acc, final_stab, final_total)

            # 4. 랭킹 보드에 최종 점수 던지기 (정렬, 하이라이트, 스크롤 자동 수행)
            # 어제 만든 RankBoard의 update_score 함수를 호출합니다.
            self.RankBoard.update_score(final_total)

            # 5. 하단 안내 문구 변경 (InstructionPanel 활용)
            current_rank = self.RankBoard.db_scores.index(final_total) + 1
            if final_total >= 50.0:
                self.instruction_panel.set_instruction("6", rank=current_rank)
            else:
                self.instruction_panel.set_instruction("7", rank=current_rank)

            print(f'[Debug] 최종 평가 완료: {final_total:.1f}점, {current_rank}위')

        except Exception as e:
            print(f'[Error][update_rank_table] 계산 중 오류: {e}')
    ...
    @Slot()
    def next_stage(self):
        """타이머가 끝날 때마다 호출되어 다음 단계로 전환"""
        self.stage += 1
        
        if self.stage == 1:
            # 1단계: 준비 완료 후 첫 번째 동작 시작
            print("수신양호")
            self.instruction_panel.set_instruction("1") # 문구 변경
            self.progressbar_panel.start_timer(10, "#FFA400", "motion") # 5초 동작 인식 시작
            self.action_recognition = True # AI 인식 활성화
            ...
            # self.exp_evaluator.add_frame(self.STAGE_CLASS_IDXS[self.action_flag-1], action_results, keypoints=keypoints_dict.get(self.active_user_id, None))
        
        elif self.stage == 2:
            # 대기시간
            self.instruction_panel.set_instruction("2")
            self.progressbar_panel.start_timer(10, 'white', "start") # 5초 대기
            self.canvas.set_visualization_mode(show_bbox=True, show_pose=True, show_action=False)
            ...
            # 2단계 : 2번 동작
            self.instruction_panel.set_instruction("3")
            self.progressbar_panel.start_timer(10, "#FFA400", "motion")
            # 여기서 이전 단계 점수를 표에 업데이트
            self.canvas.set_visualization_mode(show_bbox=True, show_pose=True, show_action=True)
            # self.action_flag = 2
            # action_results = result_dict.get("action_results", {}).get(self.active_user_id, [])
            # self.exp_evaluator.add_frame(self.STAGE_CLASS_IDXS[self.action_flag-1], action_results, keypoints=keypoints_dict.get(self.active_user_id, None))
            ...
            # 대기시간
            self.instruction_panel.set_instruction("4")
            self.progressbar_panel.start_timer(10, 'white', "start") # 5초 대기
            self.canvas.set_visualization_mode(show_bbox=True, show_pose=True, show_action=False)
            stage_result = self.exp_evaluator.end_stage(self.STAGE_CLASS_IDXS[self.action_flag-1])
            ...
            # 3단계 : 3번 동작 
            self.instruction_panel.set_instruction("5")
            self.progressbar_panel.start_timer(10, "#FFA400", "motion")
            self.canvas.set_visualization_mode(show_bbox=True, show_pose=True, show_action=True)
            # self.action_flag = 3
            # action_results = result_dict.get("action_results", {}).get(self.active_user_id, [])
            ...
            # 4단계: 세 번째 동작 완료 후 결과 화면으로 전환
            # self.instruction_panel.set_instruction("6", rank=self.rank)
            # self.progressbar_panel.start_timer(5, "#FFFFFF", "start") # 결과 화면 표시
            # self.action_recognition = False # AI 인식 비활성화
            self.canvas.set_visualization_mode(show_bbox=True, show_pose=True, show_action=False)
            stage_result = self.exp_evaluator.end_stage(self.STAGE_CLASS_IDXS[self.action_flag-1])
        ...
        # elif self.stage == 7:
        #     # 4단계: 세 번째 동작 완료 후 결과 화면으로 전환
        #     self.instruction_panel.set_instruction("4", rank=self.rank)
        #     self.update_rank_table() # 점수 및 랭킹 업데이트
        #     self.progressbar_panel.start_timer(5, "#FFFFFF", "result") # 결과 화면 표시
    ...
    def update_controller(self, result_dict):
        ...
        if self.active_user_id == None:
            self.stage = 0
            self.action_flag = 0
            # print('[Debug][update_controller] active_user_id가 없습니다. stage를 0으로 초기화 합니다.')
            ...
            if self.current_view_mode == 'webcam':
                ...
                if self.stage == 0:
                    ...
                    if self.action_recogniton:
                        ...
                        # print(f'[debug] keypoints_id : {keypoints_id.keys()}, keypoints_id shape : {keypoints_id[list(keypoints_id.keys())[0]].shape if keypoints_id else None}, active_user_id : {self.active_user_id}')
                        # print(f'[debug] keypints_id[{self.active_user_id}] {keypoints_id[self.active_user_id]}')
        ...
        result_dict['current_view_mode']=self.stage
        
        # self.canvas.update_frame(frame, self.countdown)
        self.canvas.update_frame(result_dict, self.countdown)
            
        view_mode = { 'mode' : self.current_view_mode , 'action_recognition' : self.action_recognition}
